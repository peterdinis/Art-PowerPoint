...
export default function PresentationPage() {
  ...
  const presentationId = typeof params.id === 'string' ? params.id : params.id?.[0];
...
function PresentationElement({ element, slideIndex }: { element: SlideElement; slideIndex: number }) {
  ...
  return (
    <div
      style={style}
      className={cn('select-none', getAnimationClass())}
    >
      {renderContent()}
    </div>
  );
