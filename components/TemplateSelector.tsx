...
import { X, Sparkles, Briefcase, GraduationCap, Palette, Minus } from 'lucide-react';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { templates, type PresentationTemplate } from '@/lib/templates/presentationTemplates';
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor: template.slides[0]?.background?.gradient
                            ? undefined
                            : (template.slides[0]?.background?.color || '#ffffff'),
                          background: template.slides[0]?.background?.gradient || undefined,
                      ...
                      >
                        ...
                        <div className="absolute inset-0 p-4">
                          {template.slides[0]?.elements.slice(0, 3).map((el) => {
                            if (el.type === 'text') {
                              return (
                                <div
                                  key={el.id}
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    ...
                                    borderRadius: el.content === 'circle' ? '50%' : el.content === 'rounded' ? '8px' : '0',
                                    clipPath: el.content === 'triangle' ? 'polygon(50% 0%, 0% 100%, 100% 100%)' : undefined,
                                  }}
                                />
                              );
                            }
                        ...
                        <div className="absolute bottom-2 left-2 bg-background/80 backdrop-blur-sm text-xs px-2 py-1 rounded">
                          {template.slides.length} slajd{template.slides.length !== 1 ? 'ov' : ''}
                        </div>
                      </div>
                      <CardHeader>
                        <CardTitle className="group-hover:text-primary transition-colors">
