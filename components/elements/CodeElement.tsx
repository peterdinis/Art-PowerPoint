...
export default function CodeElement({ element, isSelected }: CodeElementProps) {
    ...
    const getBackgroundStyle = () => {
        ...
        return {
            backgroundColor: element.style?.backgroundColor || (theme === "dark" ? "#1e1e1e" : "#f5f5f5")
        };
    };
    ...
    return (
        <div
            ...
            style={{
                ...getBackgroundStyle(),
                color: element.style?.color || (theme === "dark" ? "#d4d4d4" : "#333333"),
        ...
        >
            <div className="flex items-center justify-between mb-2 text-xs opacity-50 border-b border-white/10 pb-1">
                <span>{language}</span>
                {theme === "dark" && <span className="w-2 h-2 rounded-full bg-yellow-500" />}
            </div>
