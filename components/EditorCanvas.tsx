...
export default function EditorCanvas() {
  const { currentPresentation, currentSlideIndex, selectedElementId, selectElement, updateElement } =
  ...
  const [{ isOver }, drop] = useDrop({
    ...
    drop: (item: { id: string }, monitor) => {
      if (!monitor.didDrop() && currentPresentation && dropRef.current) {
        ...
        if (delta) {
          const element = currentPresentation.slides[currentSlideIndex]?.elements.find(
            (el) => el.id === item.id
          );
          if (element) {
            ...
            const newX = Math.max(0, Math.min(960 - element.size.width, element.position.x + delta.x / scaleX));
            const newY = Math.max(0, Math.min(540 - element.size.height, element.position.y + delta.y / scaleY));
    ...
    <div className="flex items-center justify-center h-full p-4 lg:p-8 overflow-auto">
      <div className="relative w-full max-w-full flex items-center justify-center">
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget || (e.target as HTMLElement).classList.contains('slide-background')) {
              selectElement(null);
            }
          }}
        >
              ...
              <div className="flex flex-col items-center gap-4 p-8">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md">
                    Kliknite na tlačidlo "Pridať" v paneli nástrojov alebo použite rýchle tlačidlá
                  </p>
                </div>
              </div>
            </div>
