...
async function parseOdpContent(
	...
	const _stylesDoc = stylesXmlText
		? parser.parseFromString(stylesXmlText, "text/xml")
		: null;
	...
	const autoStyles = contentDoc.getElementsByTagName(
	...
	if (autoStyles) {
		...
		for (let i = 0; i < pageLayouts.length; i++) {
			...
			const props = layout.getElementsByTagName(
		...
		for (let i = 0; i < drawingStyles.length; i++) {
			...
			if (style.getAttribute("style:family") === "drawing-page") {
				...
				const props = style.getElementsByTagName(
	...
	for (let pIndex = 0; pIndex < pages.length; pIndex++) {
		...
		const styleName =
			...
			page.getAttribute("draw:master-page-name");
